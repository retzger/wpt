var validator = {
  ...
  test_willValidate: function(ctl, data) {
    ...
    test(function () {
      ...
      self.set_conditions(ctl, data.conditions);
      assert_true(
          !data.testDisabledAndReadonly,
          'testDisabledAndReadonly not supported for this test');
      if (data.ancestor) {
  ...
  iterate_over: function(ctl, data) {
    if (data.testDisabledAndReadonly === undefined ||
        data.testDisabledAndReadonly) {
      ...
      return [{ctl: ctl, data: data, condStr: '.'}];
    }
  },
